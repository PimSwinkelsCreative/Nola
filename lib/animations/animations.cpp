    ...
    float intensity)
{
    ...
    RGBWColor16 baseColor = fadeColor(RGBWColor16(0, 0, 0, 0), color, HEARTBEAT_BASE_INTENSITY);
    RGBWColor16 peakColor = fadeColor(RGBWColor16(0, 0, 0, 0), color, HEARTBEAT_PEAK_INTENSITY);
    uint16_t fadeTime = uint16_t(float(heartBeatPeriod) * HEARTBEAT_PULSE_DURATION);
    float fadePercentage = float(animationTime % fadeTime) / float(fadeTime);

    RGBWColor16 outputColor = baseColor;

    if (animationTime <= fadeTime || (animationTime > 2 * fadeTime && animationTime <= 3 * fadeTime)) {
        // rising edges of the heartbeat:
        outputColor = fadeColor(baseColor, peakColor, fadePercentage);

    } else if ((animationTime > fadeTime && animationTime <= 2 * fadeTime) || (animationTime > 3 * fadeTime && animationTime <= 4 * fadeTime)) {
...
void deleteLightDot(uint8_t index)
{
    if (index < 0 || index >= nLightDots) {
        Serial.println(
            "ERROR: cannot delete LightDot. Index out of range! index: " + String(index));
        return;
    }
...
{
    uint16_t animationTime = (millis() - animationStartTime) % SHYLIGHTS_ANIMATION_TIME;
    if (animationTime < shyLightsFadeTimes[address][0]) {
        // handle the fadeIn
        float brightness = constrain(float(animationTime) / float(SHYLIGHTS_START_FADEIN), 0, 1);
        // determine if the orange color should be fully on
        uint8_t fullyOrangeChannels = animationTime / (shyLightsFadeTimes[address][0] / N_LEDS);
        for (int i = 0; i < N_LEDS; i++) {
            ...
            if (i <= fullyOrangeChannels) {
                ...
                float channelBrightness = float(animationTime % (shyLightsFadeTimes[address][0] / N_LEDS)) / float(shyLightsFadeTimes[address][0] / N_LEDS);
    ...
    } else if (animationTime < (shyLightsFadeTimes[address][0] + shyLightsFadeTimes[address][1])) {
        // handle the fadeOut, always have white and orange both on
        RGBWColor16 heartbeatColor = SHYLIHGTS_WHITE_COLOR;
        heartbeatColor += SHYLIGHTS_ORANGE_COLOR;
        float brightness = 1.0 - float(animationTime - shyLightsFadeTimes[address][0]) / float(shyLightsFadeTimes[address][1]);
        for (int i = 0; i < N_LEDS; i++) {
            updateHeartBeat(heartbeatColor, i, HEARTBEAT_BPM, brightness);
        }
    } else {
...
void updateBreathingAnimation(uint8_t address)
{
    ...
    uint32_t animationTime = (millis() - animationStartTime) % (BREATHING_ANIMATION_LENGTH * 1000);

    // determine if there is a fade active atm:
    uint32_t totalFadeTime = BREATHING_FADE_IN_LENGTH + BREATHING_FADE_OUT_LENGTH;
    static int32_t lastFadeStarted = -1;

    animationCounter = animationTime / 1000; // set the animation counter to 1Hz intervals

    if ((lastFadeStarted < 0) && breathingQueues[address][animationCounter] > 0) {
        lastFadeStarted = animationTime;
        Serial.println(lastFadeStarted);
    }

    if (lastFadeStarted >= 0) {
        if (animationTime - lastFadeStarted < BREATHING_FADE_IN_LENGTH) {
            // handle the fade in
            float brightness = float(animationTime - lastFadeStarted) / float(BREATHING_FADE_IN_LENGTH);
            setAllLedsTo(fadeColor(baseColor, peakColor, brightness));
        } else if (animationTime - lastFadeStarted < BREATHING_FADE_IN_LENGTH + BREATHING_FADE_OUT_LENGTH) {
            // handle the fade out
            float brightness = float(animationTime - lastFadeStarted - BREATHING_FADE_IN_LENGTH) / float(BREATHING_FADE_OUT_LENGTH);
            setAllLedsTo(fadeColor(peakColor, baseColor, brightness));
        } else {
            lastFadeStarted = -1;
...
void updateTwoColorRotationAnimation(RGBWColor16 foregroundColor, RGBWColor16 backgroundColor, float animationDuration, float rotationAmount)
{
    ...
    float animationDirection = rotationAmount * sin(2 * PI * float(millis()) / (animationDuration));
...
void updateColorFade(uint16_t fadeTime, RGBWColor16 colors[], uint8_t nColors)
{
    ...
    if (nColors == 1) {
        ...
        uint16_t animationTime = (millis() - animationStartTime) % (fadeTime * nColors);
        uint8_t currentColor = animationTime / fadeTime;
        uint16_t currentFadeTime = animationTime % fadeTime;

        // Serial.println("Current Color: " + String(currentColor) + "\tCurrentFadeTime: " + String(currentFadeTime) + "\tanimationTime: " + String(animationTime));
        outputColor = fadeColor(colors[currentColor], colors[(currentColor + 1)%nColors], float(currentFadeTime) / float(fadeTime));
    }
    setAllLedsTo(outputColor);
}
