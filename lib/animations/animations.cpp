...
uint32_t fadeStarted = 0;
uint16_t fadeInTime = 0;
uint16_t fadeFullTime = 0;
uint16_t fadeOutTime = 0;
...
uint16_t getAnimationBrightness() {
  ...
  uint16_t brightness;
  uint32_t fadeTime = millis() - fadeStarted;
  if (fadeTime < fadeInTime) {
    brightness = map(fadeTime, 0, fadeInTime, 0, maxColorValue);
  } else if (fadeTime < fadeInTime + fadeFullTime) {
    brightness = maxColorValue;
  } else if (fadeTime < fadeInTime + fadeFullTime + fadeOutTime) {
    brightness = map(fadeTime - fadeInTime - fadeFullTime, 0, fadeOutTime,
                     maxColorValue, 0);
  } else {
    brightness = 0;
    fadeActive = false;
...
void updateBreathingAnimation(uint8_t address) {
  ...
  animationCounter =
      animationTime / 1000;  // set the animation counter to 1Hz intervals
  ...
  if (lastFadeStarted >= 0) {
    if (animationTime - lastFadeStarted < BREATHING_FADE_IN_LENGTH) {
      // handle the fade in
      float brightness = float(animationTime - lastFadeStarted) /
                         float(BREATHING_FADE_IN_LENGTH);
      setAllLedsTo(fadeColor(baseColor, peakColor, brightness));
    } else if (animationTime - lastFadeStarted <
               BREATHING_FADE_IN_LENGTH + BREATHING_FADE_OUT_LENGTH) {
      // handle the fade out
      float brightness =
          float(animationTime - lastFadeStarted - BREATHING_FADE_IN_LENGTH) /
          float(BREATHING_FADE_OUT_LENGTH);
...
void updateTwoColorRotationAnimation(RGBWColor16 foregroundColor,
  ...
  float animationDirection =
      rotationAmount * sin(2 * PI * float(millis()) / (animationDuration));
...
void updateColorFade(uint16_t fadeTime, RGBWColor16 colors[], uint8_t nColors) {
  ...
  if (nColors == 1) {
    ...
    uint16_t animationTime =
        (millis() - animationStartTime) % (fadeTime * nColors);
    ...
    // "\tCurrentFadeTime: " + String(currentFadeTime) + "\tanimationTime: " +
    // String(animationTime));
    outputColor =
        fadeColor(colors[currentColor], colors[(currentColor + 1) % nColors],
                  float(currentFadeTime) / float(fadeTime));
  }
